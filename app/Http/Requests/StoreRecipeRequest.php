...
class StoreRecipeRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'meal_times.*.id' => ['required', 'integer', 'exists:meal_times,id'],
            'meal_times.*.slug' => ['required', 'string', 'exists:meal_times,slug'],
            'ingredients' => ['required', 'array'],
            'ingredients.*.name' => ['required', 'string', 'max:255'],
            'ingredients.*.quantity' => ['required', 'numeric', 'min:0'],
            'ingredients.*.unit' => ['required', 'string', 'max:255'],
            'ingredients.*.category_id' => ['required', 'integer', 'exists:ingredient_categories,id'],
