...
class RecipeAIGenerationAction
{
     ...
     */
    private function buildMessages(RecipeAIGenerationRequestData $promptData): array
    {
        return [
            [
                'role' => 'system',
                ...
                - serving_size: Nombre de portions que produit cette recette (entre 1 et 50, choisit selon le type de plat et les quantités d'ingrédients)
            ...
            if (isset($response->choices[0]->message->toolCalls)) {
                foreach ($response->choices[0]->message->toolCalls as $toolCall) {
                    if ($toolCall->function->name === 'generate_recipe') {
                        ...
                        return RecipeStoreRequestData::validateAndCreate($args);
                    }
                }
            }
