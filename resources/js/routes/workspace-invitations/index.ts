...
export const destroy = (
  args:
    | { invitation: string | number | { id: string | number } }
    | [invitation: string | number | { id: string | number }]
    | string
    | number
    | { id: string | number },
...
acceptFromEmail.url = (
  ...
  return (
    acceptFromEmail.definition.url
      .replace('{token}', parsedArgs.token.toString())
      .replace(/\/+$/, '') + queryParams(options)
  );
};
